...
bool uartInit(void)
{
  ...
  for (int i = 0; i < UART_MAX_CH; i++)
  {
    ...
    case _DEF_UART3:
      ...
      uart_tbl[i].p_tx_buf = &uart3_tx_data[0];
#endif
      uart_tbl[i].p_huart = &huart3;
      uart_tbl[i].p_hdma_rx = &hdma_usart3_rx;
      break;
    default:
      break;
    }
...
{
  uint32_t ret = 0;
  char print_buf[UART_BUF_LENGTH];

  int len;
  len = vsnprintf(print_buf, UART_BUF_LENGTH, fmt, arg);
...
{
  char buf[UART_BUF_LENGTH];
  va_list args;
  int len;
  uint32_t ret;

  va_start(args, fmt);
  len = vsnprintf(buf, UART_BUF_LENGTH, fmt, args);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART1)
  {
    ...
    HAL_UART_Receive_IT(uart_tbl[_DEF_UART1].p_huart, (uint8_t *)&uart_tbl[_DEF_UART1].rx_data, 1);
#endif
  }
